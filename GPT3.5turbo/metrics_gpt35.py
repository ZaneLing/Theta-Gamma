# metrics.py
# Evaluation utilities for answer/support metrics across datasets.
...
def answer_em(dataset_name: str, pred: str, golds: List[str]) -> float:
    """
    Overlap-based EM: if pred and any gold share any overlap (equal, substring, or token intersection),
    return 1 else 0. Applies to all datasets.
    """
...
def answer_f1(pred: str, golds: List[str]) -> float:
    """
    Overlap-based F1 for all datasets: if there is any overlap (equal, substring, or token intersection),
    return 1 else 0.
    """
    if not pred or not golds:
        return 0.0
...
def extract_predicted_support_indices(gamma_results: List[Dict[str, Any]]) -> List[int]:
    """
    Collect unique fact indices selected by gamma across steps.
    """
