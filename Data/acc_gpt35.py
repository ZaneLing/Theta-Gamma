...
class ACCAgent:
    ...
    # ------------------------------------------------------------------
    # 符号层：简单“same country / same nationality” 判断（优先修掉方向错）
    # ------------------------------------------------------------------
    def _normalize_country(self, name: str) -> str:
        """
        粗粒度国家归一化：
        - American / United States / U.S. -> "usa"
        - British / United Kingdom / England -> "uk"
        - 其他国家简单小写去符号。
        """
    ...
    def _symbolic_bool_from_evidence(
        self,
        question: str,
        gamma_results: List[Dict[str, Any]],
    ) -> Optional[str]:
        """
        对“是否来自同一国家 / 同一国籍”类 yes/no 问题做一个符号层判断。
        返回:
            - "yes" / "no" -> 明确结论
            - None         -> 无法判断，交给 LLM
        """
        q_lower = question.lower()
        # 只处理“same country / same nationality / same state”类问题
        if not any(kw in q_lower for kw in ["same country", "same nationality", "same state"]):
            return None

        # 从 gamma 的子问题里抓所有 "country / nationality / state" 的答案
        raw_countries: List[str] = []
        for step in gamma_results:
            subq = (step.get("refined_subquestion") or step.get("subquestion") or "").lower()
            if any(kw in subq for kw in ["country", "nationality", "state"]):
                gres = step.get("gamma_result", {}) or {}
                ans = gres.get("answer")
                if isinstance(ans, str) and ans.strip():
                    raw_countries.append(ans.strip())

        if len(raw_countries) < 2:
            # 证据太少，不做符号判断
            return None

        norm_countries = [self._normalize_country(c) for c in raw_countries]
        norm_countries = [c for c in norm_countries if c]  # 去掉空
        if len(norm_countries) < 2:
            return None

        uniq = set(norm_countries)
        # 如果有至少两个不同的国家名：显然“不是同一国家”
        if len(uniq) >= 2:
            return BOOL_NO
        # 如果所有归一化后都是同一个国家，且至少两条证据：显然“是同一国家”
        if len(uniq) == 1 and len(norm_countries) >= 2:
            return BOOL_YES
    ...
    def check_answer(
        ...
        if is_bool_q:
            ...
            init_norm_for_symbol = self._normalize_bool_answer(initial_answer)
            initial_answer_normed = init_norm_for_symbol
        else:
        ...
        # 2) 先跑一遍“符号层 yes/no 判断”，优先修掉明显方向错
        if is_bool_q:
            symbolic_bool = self._symbolic_bool_from_evidence(question, gamma_results)
        else:
            symbolic_bool = None

        if is_bool_q and symbolic_bool in {BOOL_YES, BOOL_NO}:
            # 有明确符号结论：对比一下 Theta 的答案
            init_norm = self._normalize_bool_answer(initial_answer)
            # Theta 自己也是明确 yes/no
            if init_norm in {BOOL_YES, BOOL_NO}:
                if symbolic_bool != init_norm:
                    # 方向错，很明显：直接在本地做 FLIP_BOOL，不问 LLM
                    explanation = (
                        f"Symbolic check: question asks about 'same country/nationality', "
                        f"Gamma evidence shows different normalized countries => "
                        f"the correct boolean should be '{symbolic_bool}', "
                        f"so flip Theta's '{init_norm}' to '{symbolic_bool}'."
                        if symbolic_bool == BOOL_NO
                        else
                        f"Symbolic check: Gamma evidence shows all entities share the same normalized country "
                        f"=> boolean should be '{symbolic_bool}', flip Theta's '{init_norm}'."
                    )
                    return {
                        "action": ACTION_FLIP_BOOL,
                        "final_answer": symbolic_bool,
                        "explanation": explanation,
                        "flags": {
                            "entity_mismatch": False,
                            "logic_inconsistency": True,
                            "insufficient_evidence": False,
                        },
                        "candidates": candidates,
                        "raw_llm_output": "",
                        "raw_json": {"symbolic_bool": symbolic_bool, "initial": init_norm},
                    }
                else:
                    # 符号结论与 Theta 一致：直接 KEEP，就不再浪费 LLM 调用
                    return {
                        "action": ACTION_KEEP,
                        "final_answer": init_norm,
                        "explanation": (
                            "Symbolic check: Theta's yes/no answer is consistent with "
                            "Gamma evidence for 'same country/nationality' question."
                        ),
                        "flags": {
                            "entity_mismatch": False,
                            "logic_inconsistency": False,
                ...
                # Theta 原答案不是明确 yes/no，但符号结论给出了 yes/no：
                # 可以选择 CHOOSE_FROM_CANDIDATES 或直接作为 final_answer。
                explanation = (
                    "Symbolic check: Gamma evidence implies a clear yes/no answer, "
                    "while Theta's initial answer is not a clear boolean."
                )
                return {
                    "action": ACTION_CHOOSE if candidates else ACTION_KEEP,
                    "final_answer": symbolic_bool,
                    "explanation": explanation,
                    "flags": {
                        "entity_mismatch": False,
                    ...
                    "candidates": candidates,
                    "raw_llm_output": "",
                    "raw_json": {"symbolic_bool": symbolic_bool, "initial": initial_answer},
                }

        # 3) 符号层无法判断：再走 LLM 的有限 action 逻辑
